...
Buttons can include icons using the `icon` and `iconPlacement` props, or by placing SVG elements directly as children.

<div className='flex flex-wrap items-center gap-4 rounded-lg border p-6 my-4'>
  <Button icon={Mail} iconPlacement='left'>
    Send Email
  </Button>
...
```tsx
import { Mail, ArrowRight, Download, Trash2, Heart } from 'lucide-react';

<Button icon={Mail} iconPlacement="left">Send Email</Button>
<Button icon={ArrowRight} iconPlacement="right">Next</Button>

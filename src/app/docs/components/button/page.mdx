...
import { StyleShowcase } from '@/components/docs/style-showcase';

export const metadata = {
  title: 'Button',
  description:
    'A versatile button component with multiple variants, sizes, visual effects, icons, and loading states. Built on Radix UI and Tailwind CSS.',
...
<JsonLd
  data={{
    ...
    '@type': 'TechArticle',
    headline: 'Button — Prisma UI',
    description:
      'A versatile button component with multiple variants, sizes, visual effects, icons, and loading states.',
    breadcrumb: {
      '@type': 'BreadcrumbList',
      itemListElement: [
        {
        ...
        },
        {
          ...
          position: 3,
          name: 'Button',
          item: 'https://prismaui.com/docs/components/button',
        },
      ],
    },
  }}
...
The direct URL serves the `radix-vega` variant. The Button is style-aware — its sizes, radius, and type scale differ per shadcn style. To let the CLI pick the variant that matches your project's theme automatically, register the `@prisma` namespace once (see [Styles & Themes](/docs/styles)) and install by name:
...
<Button variant="secondary" effect="shineHover">Shine Hover</Button>
```
...
<ComponentPreview>
  <Button className='ring-offset-muted' effect='ringHover'>
    Ring Hover
  </Button>
  <Button
    className='ring-offset-muted'
    variant='destructive'
    effect='ringHover'
  >
    Ring Hover
  </Button>
  <Button className='ring-offset-muted' variant='outline' effect='ringHover'>
    Ring Hover
  </Button>
</ComponentPreview>
...
<Button variant="outline" effect="ringHover">Ring Hover</Button>
```
...
<Button effect="gooeyLeft" variant="destructive">Gooey Left</Button>
```
...
```tsx
<Button effect="pulsating">Pulsating</Button>
<Button variant="destructive" effect="pulsating">Pulsating</Button>
```
...
```tsx
<Button effect="rainbow">Rainbow</Button>
<Button size="lg" effect="rainbow">Rainbow Large</Button>
```
...
The Button ships in a dedicated variant for each supported shadcn style. Installed through the `@prisma` namespace, the CLI delivers the one matching the `style` in your `components.json` — same API, different sizing, radius, and type scale:

<StyleShowcase component='button' />

See [Styles & Themes](/docs/styles) for setup and the full list of supported styles.

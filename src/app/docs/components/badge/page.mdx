...
The direct URL serves the `radix-vega` variant. The Badge is style-aware — its sizes, radius, and type scale differ per shadcn style. To let the CLI pick the variant that matches your project's theme automatically, register the `@prisma` namespace once (see [Styles & Themes](/docs/styles)) and install by name:

<InstallSnippet registryUrl='@prisma/badge' />
